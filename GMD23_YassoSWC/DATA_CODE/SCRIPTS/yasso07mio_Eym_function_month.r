...
# and runs on the modeling platform of SoilR package (Sierra et al.2012) including also its own Yasso07Model version (Sierra, C.A., M?ller, M. and Trumbore, S.E., 2012. Models of soil organic matter decomposition: the SoilR package, version 1.0. Geoscientific Model Development, 5, pp.1045-1060). 
...
#  and for SOC time series at yearly (Y) or monthly (M) time steps
##################################################################
...
## TEST MONTH MODIFICATION FROM 23.02.20 VERSION => ALWAYS REDUCE DECOMPOSITION RATES BY 12, INPUT ANNUAL LITTER AS MONTHLY MEAN !!!!!!!!!!  #############################
# core function #############
Yasso07mio_Month <- function( t.step, # 0 = Equilibrium SOC, 1 = year or 1/12 = month #t.step months reduces the decomposition rates by 12
                            ...
                            parameters
                            ){ #vector of all parameters, same as in the model description of Tuomi et al. 2011,except for environmental climate parameters
  ...
  ksY = as.numeric(parameters[1:5])/12 
  ...
  if(t.step==0){
        #equilibrium soc solve analyticaly Sierra et al. (2018) : Sierra, C.A., Ceballos-Nuez, V., Metzler, H. and M?ller, M., 2018. Representing and understanding the carbon cycle using the theory of compartmental dynamical systems. Journal of Advances in Modeling Earth Systems, 10(8), pp.1729-1734.
        #xss = -B^{-1}(t)*u(t)
...
cat("\n Yasso07mio model: the model function 'Yasso07mio_Month' (for testing equilibrium SOc with monthly litter and monthly decay rates) and parameters 'ym.mean.params' were loaded, for simple examples of data input and model simulation run Yasso07mio.EYM.examples() ")
